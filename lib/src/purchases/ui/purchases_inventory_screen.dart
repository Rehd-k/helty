...
class PurchasesInventoryScreen extends StatefulWidget {
  const PurchasesInventoryScreen({super.key});

  @override
  State<PurchasesInventoryScreen> createState() =>
      _PurchasesInventoryScreenState();
}

class _PurchasesInventoryScreenState extends State<PurchasesInventoryScreen> {
  final PurchasesApiService _drugService = PurchasesApiService();

  List<PurchaseItem> _drugs = [];
  ...
  final Map<String, List<ItemLocationQuantity>> _drugLocationQuantities = {};
  ...
  Future<void> _loadFilterOptions() async {
    ...
    try {
      final manu = await _drugService.getManufacturers(
        const PurchasesQueryParams(pageSize: 500),
      );
  ...
  SearchPurchaseItemParams _buildSearchParams() {
    ...
    return SearchPurchaseItemParams(
      ...
      itemName: query.isEmpty ? null : query,
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final response = await _drugService.searchItems(_buildSearchParams());
  ...
  Future<void> _fetchDrugLocationQuantities(String itemId) async {
    if (itemId.trim().isEmpty) return;
    if (_loadingDrugLocationIds.contains(itemId)) return;

    setState(() {
      _loadingDrugLocationIds.add(itemId);
      _drugLocationErrors.remove(itemId);
    });

    try {
      final locations = await _drugService.getItemLocationQuantities(itemId);
      if (!mounted) return;
      setState(() {
        _drugLocationQuantities[itemId] = locations;
      });
    } on AppException catch (e) {
      if (!mounted) return;
      setState(() {
        _drugLocationErrors[itemId] = e.message;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _drugLocationErrors[itemId] = e.toString();
      });
    } finally {
      if (mounted) {
        setState(() {
          _loadingDrugLocationIds.remove(itemId);
        });
      }
    }
  }

  void _selectDrug(PurchaseItem item) {
    setState(() {
      _selectedDrug = item;
  ...
  Widget _buildDetailsPanel(ThemeData theme, PurchaseItem item) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _buildStatusChip(item.displayStatus),
              const SizedBox(width: 8),
              if (item.displayStatus == 'Expiring Soon' &&
                  item.expiryDate != null)
                Container(
          ...
          _buildInfoCard(theme, 'Stock Locations', Icons.storefront_outlined, [
            ..._buildStockLocationChildren(theme, item),
          ]),
          const SizedBox(height: 24),
          SizedBox(
            width: double.infinity,
            child: FilledButton.icon(
              onPressed: () => _showOrderModal(context, theme, item),
              icon: const Icon(Icons.shopping_cart_outlined, size: 20),
              label: Text('Order ${item.itemName}'),
  ...
  List<Widget> _buildStockLocationChildren(ThemeData theme, PurchaseItem item) {
    final id = item.id;
    ...
    final locations =
        _drugLocationQuantities[id] ?? const <ItemLocationQuantity>[];
    ...
    showDialog<void>(
      ...
      builder: (ctx) => _AddMedicineDialog(
        ...
        onSaved: () {
          _fetchData();
          _loadFilterOptions();
          if (_selectedDrug?.id == item.id) {
            setState(() {
              _selectedDrug = _drugs.cast<PurchaseItem?>().firstWhere(
                (d) => d?.id == item.id,
...
class _AddMedicineDialog extends StatefulWidget {
  ...
  final PurchasesApiService drugService;
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      ...
      child: SizedBox(
        width: 800,
        height: 600,
        child: PurchasesAddItemScreen(
          existingItem: widget.existingItem,
          service: widget.drugService,
          onSaved: widget.onSaved,
          key: ValueKey(isEdit ? 'edit-item-dialog' : 'add-item-dialog'),
        ),
      ),
    );
  }
