                  ...
                  children: [
                    ...
                    LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        final cards = [
                          for (final status in HospitalAssetStatus.values)
                            StatusCountCard(
                              status: status,
                    ...
                    SingleChildScrollView(
                      ...
                      child: Row(
                        children: [
                          ...
                          for (final k in HospitalAssetKind.values) ...[
                            FilterChip(
                              avatar: Icon(iconForAssetKind(k), size: 16),
                              label: Text(k.label),
                              selected: _kind == k,
                              onSelected: (_) {
                                setState(() => _kind = k);
                      ...
                      SingleChildScrollView(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 8),
                            for (final dept in kInventoryDepartments) ...[
                              FilterChip(
                                label: Text(dept.label),
                                selected: _departmentFilter == dept.apiValue,
                                selectedColor: colorForAccountType(
                                  dept.apiValue,
                                ).withValues(alpha: 0.22),
                                side: BorderSide(
                                  color: colorForAccountType(dept.apiValue),
                                ),
                                onSelected: (_) {
                                  setState(
                                    () => _departmentFilter = dept.apiValue,
                                  );
                                  _load();
                                },
                              ),
                        ...
                        child: _visible.isEmpty
                            ? ListView(
                                children: const [
                                  ...
                                  SizedBox(height: 12),
                                  Center(child: Text('No assets in this view')),
                                ],
                              )
                            : ListView(
                                children: [
