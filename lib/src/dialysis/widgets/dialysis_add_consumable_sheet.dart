...
Future<DialysisSessionConsumable?> showDialysisAddConsumableSheet({
  required BuildContext context,
  required WidgetRef ref,
  required String sessionId,
  required String patientId,
}) {
  return showModalBottomSheet<DialysisSessionConsumable>(
    context: context,
...
class _DialysisAddConsumableSheet extends ConsumerStatefulWidget {
  const _DialysisAddConsumableSheet({
  ...
  @override
  ConsumerState<_DialysisAddConsumableSheet> createState() =>
      _DialysisAddConsumableSheetState();
}

class _DialysisAddConsumableSheetState
    extends ConsumerState<_DialysisAddConsumableSheet> {
  ...
  void _onSearchChanged(String q) {
    ...
    _debounce = Timer(const Duration(milliseconds: 350), () async {
      ...
      try {
        ...
        if (!mounted) return;
        setState(() {
          _hits = res.items.where((c) => c.isBillable).toList();
          _searching = false;
        });
      } catch (_) {
        if (!mounted) return;
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final row = await api.addSessionConsumable(
        widget.sessionId,
        consumableId: sel.id!,
      ...
      await _submitViaClientFallback(
        consumableId: sel.id!,
        storeLocationId: loc.id,
        quantity: qty,
        unitPrice: unit,
      ...
      await _submitViaClientFallback(
        ...
        quantity: qty,
        unitPrice: unit,
        priorError: e.toString(),
      );
    }
  }

  Future<void> _submitViaClientFallback({
    ...
    try {
      final storeApi = StoreConsumableApiService();
      await storeApi.recordUsage(
      ...
      if (!mounted) return;
      final synthetic = DialysisSessionConsumable(
        ...
        consumable: DialysisConsumableRef(
          id: consumableId,
          name: _selected?.name,
        ),
      );
      Navigator.of(context).pop(synthetic);
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _submitting = false;
        _error = priorError != null
